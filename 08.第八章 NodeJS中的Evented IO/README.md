...
NodeJS利用V8(Chrome JavaScript引擎)提供高性能的服务器环境。Node不仅限于Web服务器的范围 - 这只是存在于

构思上的原始问题。事实上，它的出现是为了解决Web开发者面临的一些棘手的并发问题。


本章的目的是解释Node如何处理并发，以及我们如何编程NodeJS代码以充分利用这种环境。Node和其他Web服务器环境
...
由于IO事件循环以网络请求和文件操作为基础，因此我们将在本章的后面部分中查看各种网络和文件IO示例。
...
本章后面探讨这些概念。也许这是IO事件循环带来的误导，因为它确实在一个单独的CPU上运行。
...
在一个给定的网络应用基础设施中最慢的部分是网络IO和存储IO。这些发展是相当快的，主要是在过去几年中由于对物理

硬件上的改进，但相比于正在CPU上运行的软件任务，IO是龟速的。让网络应用程序性能方面如此具有挑战性的是，存在
...
什么类型的IO操作一个给定的任务需要来执行并没有什么关系;它不会阻止其他任务运行。这是事件IO架构与运行在

一个单独的线程可以得到的方式。这是NodeJS所擅长的并发类型 - 并行的在大量IO工作中执行。但是，我们也需要
